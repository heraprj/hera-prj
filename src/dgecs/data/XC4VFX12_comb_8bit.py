...
class Individual(Individual.AbstractIndividual):
	...
	def setCell(self, pos, val):
		Co = pos[0]
		Ce = pos[1]
		self.columns[Co].getCell(Ce).setValue(val)

	def getCell(self, pos):
		Co = pos[0]
		Ce = pos[1]
		return self.columns[Co].getCell(Ce).getValue()
...
#					print arrayModule[1], arrayModule[2], arrayColumn[1], k, arrayLut[k]
	...
	def getLevels(self):
		return 3 #3 levels: Individual, Column, Cell

	def getLevel0Dimension(self):
		return 1 #The individual is an array of Columns
...
if __name__ == '__main__':
	...
	if inp == None or out == None or name == None or version == None:
		if inp == None:
			inp = 'export_luts_c/example_8.luts'
		if out == None:
			out = os.getenv("HOME")
		if comp_name == None:
